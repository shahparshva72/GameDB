...
class APIManager {
    ...
    lazy var wrapper: IGDBWrapper = .init(
        proxyURL: Constants.proxyURL, proxyHeaders: [:]
    )
    ...
    private func performRequest(endpoint: Endpoint, query: String, completion: @escaping GameFetchCompletion) {
        wrapper.apiProtoRequest(endpoint: endpoint, apicalypseQuery: query) { bytes in
            guard let gameResults = try? Proto_GameResult(serializedBytes: bytes) else {
    ...
    func fetchGame(id: Int, completion: @escaping (Result<GameModel, Error>) -> Void) {
        wrapper.apiProtoRequest(endpoint: .GAMES, apicalypseQuery: "fields name, summary, genres.name, storyline, first_release_date, screenshots.image_id, id, rating, aggregated_rating, cover.image_id, involved_companies.company.name, videos.video_id, platforms.name; where id = \(id);", dataResponse: { bytes in
            guard let protoGame = try? Proto_GameResult(serializedBytes: bytes).games.first else {
                return
            }
            DispatchQueue.main.async {
...
private extension GameModel {
    init(from response: GameSearchResponse) {
        id = response.id
        name = response.name
        releaseDate = Date(timeIntervalSince1970: response.first_release_date ?? 0)
        rating = response.rating ?? 0.0
        let imageID = response.cover?.image_id ?? ""
        coverURLString = imageBuilder(imageID: imageID, size: .COVER_SMALL, imageType: .JPEG)
        storyline = ""
        summary = ""
    ...
    init(game: Proto_Game, coverSize: ImageSize = .COVER_SMALL) {
        ...
        let screenshotURLs = game.screenshots.map { scr -> String in
            let url = imageBuilder(imageID: scr.imageID, size: .SCREENSHOT_HUGE, imageType: .JPEG)
            return url
        }
